...
public class ShellJourney
{
    ...
    public void KisayolYardimiVeSolMenu() => LiveUiHarness.Run(ui =>
    {
        ...
        // Sol menu: 15 oge, her biri dogru ekrana gider ve yalnizca o oge secili olur. "Anasınıfı"
        // yalnizca anasinifi ogrencisi varsa gorunur; canli veride yoksa o oge atlanir.
        var buttons = ((Panel)window.FindName("NavigationButtons")!).Children.OfType<Button>().ToList();
        Assert.Equal(15, buttons.Count);
        buttons = buttons.Where(b => (string)b.Tag != "kindergarten" || b.Visibility == Visibility.Visible).ToList();
        var hosts = new Dictionary<string, string>
        {
            ...
            ["student-import"] = "StudentImportHost", ["devices"] = "DevicesHost", ["device-cards"] = "DeviceCardsHost", ["card-list"] = "CardListHost", ["kindergarten"] = "KindergartenHost",
