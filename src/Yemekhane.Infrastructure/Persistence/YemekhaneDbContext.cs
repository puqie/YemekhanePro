...
public sealed class YemekhaneDbContext(DbContextOptions<YemekhaneDbContext> options) : DbContext(options)
{
    ...
    private static void ConfigureIdentityAndOrganization(ModelBuilder b)
    {
        ...
        b.Entity<StudentCard>(e => { e.HasIndex(x => x.CardNumber).IsUnique().HasDatabaseName("ix_student_cards_card_number"); e.HasIndex(x => new { x.StudentId, x.IsActive }); e.HasIndex(x => x.StudentId).IsUnique().HasFilter("IsActive = 1").HasDatabaseName("ux_student_cards_one_active"); e.Property(x => x.CardNumber).HasMaxLength(128).HasColumnName("card_number"); e.Property(x => x.PrintedNumber).HasMaxLength(32).HasColumnName("printed_number"); e.HasIndex(x => x.PrintedNumber).HasDatabaseName("ix_student_cards_printed_number"); e.HasOne<Student>().WithMany().HasForeignKey(x => x.StudentId).OnDelete(DeleteBehavior.Restrict); });
        ...
        b.Entity<SchoolClass>(e => { e.HasIndex(x => x.Name).IsUnique(); e.Property(x => x.Kind).HasMaxLength(20).HasDefaultValue(ClassKinds.Normal); }); b.Entity<Section>().HasIndex(x => x.Name).IsUnique(); b.Entity<Department>().HasIndex(x => x.Name).IsUnique(); b.Entity<Job>().HasIndex(x => x.Name).IsUnique(); b.Entity<StudentGroup>().HasIndex(x => x.Name).IsUnique();
    }
    ...
    private static void ConfigureMealsAndCalendar(ModelBuilder b)
    {
        ...
        b.Entity<Holiday>(e => { e.HasIndex(x => x.Date); e.HasIndex(x => x.Name).HasDatabaseName("ix_holidays_name"); e.HasIndex(x => x.GroupId).HasDatabaseName("ix_holidays_group_id"); }); b.Entity<HolidayScope>(e => { e.HasIndex(x => new { x.HolidayId, x.ScopeType, x.ScopeId }).IsUnique(); e.HasOne<Holiday>().WithMany().HasForeignKey(x => x.HolidayId).OnDelete(DeleteBehavior.Cascade); });
