...
public sealed class ReportPdfService : IPdfService
{
    ...
    private static readonly Dictionary<ReportType, ReportDefinition> Definitions =
        ...
        {
            [ReportType.DailyAccess] = Def("Günlük Geçiş Raporu", C("Tarih", Date, 1.3), C("Öğrenci No", x => x.StudentNo), C("Ad Soyad", Name, 1.4), C("Sınıf", x => x.Class), C("Kart", x => x.CardNo), C("Öğün", x => x.MealType), C("Cihaz", x => x.Device, 1.2), C("Karar", x => x.Decision)),
            [ReportType.MealEntitlement] = Def("Yemek Hakediş Raporu", C("Tarih", Date), C("Öğrenci No", x => x.StudentNo), C("Ad Soyad", Name, 1.5), C("Sınıf", x => x.Class), C("Öğün", x => x.MealType), C("Adet", x => x.MealCount.ToString(Turkish), .6), C("Durum", x => x.Status)),
            [ReportType.StudentMealUsage] = Def("Öğrenci Yemek Kullanım Raporu", C("Tarih", Date, 1.3), C("Öğrenci No", x => x.StudentNo), C("Ad Soyad", Name, 1.5), C("Sınıf", x => x.Class), C("Öğün", x => x.MealType), C("Kart", x => x.CardNo), C("Durum", x => x.Status)),
            ...
            [ReportType.DailyCash] = Def("Günlük Kasa Raporu", C("Tarih", Date, 1.3), C("Öğrenci No", x => x.StudentNo), C("Ad Soyad", Name, 1.4), C("Kart", x => x.CardNo), C("Açıklama", x => x.Description, 1.7), C("Durum", x => x.Status), C("Tutar", Amount)),
            [ReportType.Income] = Def("Gelir Raporu", C("Tarih", Date, 1.3), C("Öğrenci No", x => x.StudentNo), C("Ad Soyad", Name, 1.4), C("Kart", x => x.CardNo), C("Açıklama", x => x.Description, 1.7), C("Durum", x => x.Status), C("Tutar", Amount)),
            [ReportType.Sms] = Def("SMS Raporu", C("Tarih", Date, 1.2), C("Öğrenci No", x => x.StudentNo), C("Ad Soyad", Name, 1.4), C("Açıklama", x => x.Description, 2.2), C("Durum", x => x.Status)),
            [ReportType.Turnstile] = Def("Turnike Raporu", C("Tarih", Date, 1.3), C("Öğrenci No", x => x.StudentNo), C("Ad Soyad", Name, 1.4), C("Kart", x => x.CardNo), C("Cihaz", x => x.Device), C("Karar", x => x.Decision), C("Sonuç", x => x.Status), C("Açıklama", x => x.Description, 1.5)),
            [ReportType.DeniedAccess] = Def("Reddedilen Geçişler Raporu", C("Tarih", Date, 1.3), C("Öğrenci No", x => x.StudentNo), C("Ad Soyad", Name, 1.4), C("Kart", x => x.CardNo), C("Öğün", x => x.MealType), C("Cihaz", x => x.Device), C("Neden", x => x.Status, 1.4)),
            [ReportType.CardMovements] = Def("Kart Hareketleri Raporu", C("Tarih", Date, 1.3), C("Öğrenci No", x => x.StudentNo), C("Ad Soyad", Name, 1.5), C("Sınıf", x => x.Class), C("Kart", x => x.CardNo), C("Durum", x => x.Status), C("Açıklama", x => x.Description, 1.4)),
            [ReportType.HolidayTransfer] = Def("Tatil ve Aktarım Raporu", C("Tarih", Date), C("Öğrenci No", x => x.StudentNo), C("Ad Soyad", Name, 1.5), C("Öğün", x => x.MealType), C("Adet", x => x.MealCount.ToString(Turkish), .6), C("Durum", x => x.Status), C("Açıklama", x => x.Description, 1.8))
        };
